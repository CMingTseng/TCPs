...
public class TCPServerRunnable implements Runnable, ObservableInterface {
    ...
    public TCPServerRunnable(int listenport, ContentObserver targetObserver) {
    ...
    @Override
    public void run() {
        Log.d(TAG, " @ Runnable run()  get current run thread : " + Thread.currentThread().getId());
        try {
            if (mSocket == null) {
