...
public class TCPClientThread extends Thread implements ObservableInterface {
    ...
    private ContentObserver mTCPClientObserver = new ContentObserver(new Handler(Looper.getMainLooper())) {
        ...
        public void onChange(boolean selfChange, Uri uri) {
            ...
            if (type == null) {
                ...
                Log.d(TAG, "Client will send  : " + property);
    ...
    public TCPClientThread(String serverip, int serverport, ContentObserver targetObserver) {
        mServerIP = serverip;
    ...
    public void run() {
        ...
        try {
            ...
            final String selfip = mSocket.getInetAddress().getHostAddress().replace("/", "");
            while (mRunning) {
                try {
                    ...
                    if (raw != null) {
                        ...
                        if (info.getType() == MessageInfo.IDENTIFY && info.getTypename().equals(MessageInfo.MESSAGE_IDENTIFY_SERVER)) {
                            first = true;
                            info = new MessageInfo(MessageInfo.IDENTIFY, MessageInfo.MESSAGE_IDENTIFY_CLIENT,selfip,mServerIP, "master");
                            ...
                            info = new MessageInfo(MessageInfo.IDENTIFY, MessageInfo.MESSAGE_IDENTIFY_CLIENT,selfip,mServerIP, "go");
                            outmessage.setData(info);
